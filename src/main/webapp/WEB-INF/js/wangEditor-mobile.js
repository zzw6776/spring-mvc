...
(function(window) {
	...
	var $ = window.Zepto, E = window.___E, gitlink = 'github.com/wangfupeng1988/wangEditor-mobile';
...
window
		.___E_mod(function(E, $) {
			...
			E.config = {
				...
				// 菜单配置
				menus : [ 'head', 'bold', 'color', 'quote', 'list', 'img',
						'happy', 'check' ],

				// 表情图标配置
				happy : [ 'http://wangeditor.github.io/expressions/1.gif',
...
window
		.___E_mod(function(E, $) {
			...
			E.fn.addTxt = function() {
				...
				var $txt = $('<div contentEditable="true" class="wangEditor-mobile-txt">'
						+ val + '</div>');
												...
												self.$txt
														...
														.each(
																function(key,
																		node) {
																	// 遍历编辑区域所有的quote
																	var $quote = $(node), styleKey = 'w_editor_quote_style', hasStyle = $quote
																			.attr(styleKey);
				...
				menus[menuId] = {
					...
					bindEvent : function(editor) {
						...
						menuData.$trigger.on('singleTap', function(e) {
							...
							// 执行命令
							self.command('InsertUnorderedList', false,
									undefined, e);
		...
		.___E_mod(function(E, $) {
			...
			E.fn.addMenuHappy = function(menuId) {
				...
				if (happyUrlArr.length > 0) {
					if (happyUrlArr[0]
							.indexOf('http://wangeditor.github.io/expressions') === 0) {
						E
								.warn('正在使用wangEdior提供的免费表情图标，它们将从 github 下载，速度很慢！！！建议将表情图标重新配置，请参见文档说明');
					}
				}
				...
				menus[menuId] = {
					...
					renderModal : function() {
						...
						$.each(happyUrlArr, function(key, value) {
							contentHtmlArr.push(itemTpl.replace(/#{imgUrl}/ig,
									value));
						});
						...
						$modal.on('singleTap', '.command-link',
								function(e) {
									if (self.checkTapTime(e,
											'happy command-link') === false) {
										return;
									}

									var $commandLink = $(e.currentTarget);
									var commandValue = $commandLink
											.attr('commandValue');

									// 执行命令
									self.command('InsertImage', false,
											commandValue, e);
				...
				menus[menuId] = {
					...
					$trigger : (function() {
						...
						if (isAndroid || isUC) {
							...
							return $('<label for="'
									+ inputFileId
									+ '"><i class="icon-wangEditor-m-picture"></i></label>');
						}
					})(),
								...
								.on(
										...
										function(e) {
											...
											var uploadImgUrl = config.uploadImgUrl
													|| '';
											var testHostname = config.testHostname
													|| 'localhost';
											...
											if (uploadImgUrl === '') {
												// 没有配置上传图片的url
												alert('没有配置 uploadImgUrl ，wangEditor 将无法上传图片。\n\n'
														+ '想要配置上传图片，请参见文档说明。\n\n'
											...
											reader.onload = function(e) {
												var base64 = e.target.result
														|| this.result, prevImgSrc, prveImgContainerId = 'div'
														+ Math.random()
																.toString()
																.slice(2), prevImgInfoId = 'info'
														+ Math.random()
																.toString()
																.slice(2), xhr, formData, timeoutId;

												// ---------- 显示预览 ----------
												if (window.URL
														&& window.URL.createObjectURL) {
													// 如果浏览器支持预览本地图片功能，则预览本地图片
													prevImgSrc = window.URL
															.createObjectURL(file);

													// 生成预览图片，设置半透明
													$focusElem
															.after('<div class="previmg-container" id="'
																	+ prveImgContainerId
																	+ '">'
																	+ '   <p class="info" id="'
																	+ prevImgInfoId
																	...
																	+ '	<img src="'
																	+ prevImgSrc
																	+ '" style="opacity:0.2; max-width:100%;"/>'
																	+ '</div>');
												} else {
												...
												function timeoutCallback() {
													...
													if (location.hostname
															.toLowerCase() === 'wangeditor.github.io') {
														// 官网demo的特殊提示
												...
												// 添加图片数据
												// 1. 图片数据要经过
												// convertBase64UrlToBlob 转换
												// 2. wangEditorMobileFile
												// 要和后台一致
												formData.append(
														'wangEditorMobileFile',
...
window.___E_mod(function(E, $) {
	...
	E.fn.customCommand = function(isRestoreNoWrapSelection, fn, e, callback) {
		...
		/*
		 * isRestoreNoWrapSelection 参数的作用： 1. 有些 command
		 * 是需要选中整个外围选区再进行操作的，一般是修改样式，例如加粗。 针对加粗这种样式操作，如果不默认选中一个选区，是看不到任何效果的。 2.
		 * 但是有些 command 一定不能选中外围选区，一般是插入操作，例如插入图片。
...
window.___E_mod(function(E, $) {
	...
	E.fn.saveSelection = function(range) {
		var self = this, _parentElem, selection, wrapRange, txt = self.$txt
				.get(0);
